        ...
        function renderImages(gridId, category, config, page = 1) {{
            ...
            setTimeout(() => {{
                ...
                pageFiles.forEach(file => {{
                    const fullPath = config.url + '/' + encodeURI(file);
                    urls.push(fullPath);
                }});
    ...
    for f in os.listdir(video_dir):
        ...
        if os.path.isfile(full_path):
            ...
            if ext in ('.mp4', '.webm', '.ogg'):
                videos.append(f)
    videos.sort()
    print(f"video: {len(videos)} files")
...
<body>
    <header>
        <div class="header-content">
            ...
            <nav class="nav-links">
                ...
                <a href="main-gallery.html">主图画廊</a>
                <a href="emote-gallery.html">表情画廊</a>
                <a href="video-gallery.html" class="active">猫歌视频</a>
            </nav>
        </div>
    </header>
    ...
    <main>
        <section class="hero-section">
            <h2>猫歌视频</h2>
            <p>欣赏甘城なつき的猫歌视频作品，包括各种翻唱和原创歌曲视频。</p>
        </section>

        <section>
            <div class="video-grid" id="video-grid">
                {''.join(f'''<div class="video-item" onclick="openLightbox('images/video/{quote(v)}')">
                    <video src="images/video/{quote(v)}" controls poster="images/video/{quote(v)}" preload="metadata"></video>
    ...
    with open('video-gallery.html', 'w', encoding='utf-8') as f:
        f.write(content)
    print("video-gallery.html generated successfully")

if __name__ == '__main__':
    generate_main_gallery()
    generate_emote_gallery()
    generate_video_gallery()
